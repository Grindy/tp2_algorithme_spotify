...
public class PlaylistDAO {
    ...
    //Ici on verifie si la playlist qui contient toutes les chanson de la bibliotheque a ete faite
    public boolean isCreePlaylistInitiale() throws SQLException {
        String sql = "SELECT COUNT(*) FROM playlist_chanson WHERE id_playlist = ?";
        try (Connection c = Connexion.getConnexion();
             PreparedStatement ps = c.prepareStatement(sql)) {
            ps.setObject(1, UUID.fromString(uuidInitial));
            try (ResultSet rs = ps.executeQuery()) {
                if (rs.next()) {
